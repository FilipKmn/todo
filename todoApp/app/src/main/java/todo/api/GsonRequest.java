...
public class GsonRequest<T> extends Request<T> {
    ...
    public byte[] getBody() {
        try {
            return this.requestBody == null ? null : this.requestBody.getBytes(PROTOCOL_CHARSET);
        } catch (UnsupportedEncodingException var2) {
            VolleyLog.wtf("Unsupported Encoding while trying to get the bytes of %s using %s", new Object[] {this.requestBody, PROTOCOL_CHARSET});
            return null;
        }
    }
